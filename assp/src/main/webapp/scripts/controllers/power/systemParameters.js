...
angular.module('psspAdminApp')
    .controller('systemParametersCtrl',
        function ($scope, $state, $http, $interval, alertService, localStorageService, $stateParams, ngDialog, $location, FileUploader, cacheService, T) {
    	$scope.tableData = [];
    	$scope.segmentList = [];
		$scope.selected = [];
...
//					$scope.data = data.returnDataList;
			        $scope.searchSegment();
					alertService.add(data["errorType"],T.T('delete_success'));
                  } else {
    	    	...
    	    	if((!$scope.isChoice) && (!$scope.isSaveCk) && (!$scope.uploadProgressFlag)){
    		    	...
    		    	var start = $interval(function(){
    		    		if(!$scope.uploadProgressFlag){
    		    			...
    		    			$http.post('./systemPara/cancelProgress');
    		    			...
    		    			$http.post('./systemPara/redProgress').success(
    	        				function(data) {
    			    		...
    			    		$interval.cancel(start);
    			    		$http.post('./systemPara/initProgress');
