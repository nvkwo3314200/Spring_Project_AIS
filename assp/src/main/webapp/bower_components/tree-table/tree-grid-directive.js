        ...
        .run([
            ...
            function ($templateCache) {
                $templateCache.put('template/treeGrid/treeGrid.html',
                    ...
                    "		<td>" +
                    "			<input type=\"checkbox\" ng-click=\"select(row.branch);\" ng-model=\"row.branch.selected\">" +
                    "		</td>"+
            ...
            function ($timeout,
                ...
                return {
                    ...
                    link: function (scope, element, attrs) {
                        ...
                        attrs.iconLeaf = attrs.iconLeaf ? attrs.iconLeaf : 'icon-file  glyphicon glyphicon-file  fa fa-life-bouy';
                        ...
                        scope.select = function (branch) {
                        	...
                        	if(branch.children) {
                        		scope.checkChildren(branch);
                        	}
                        	
                        	if(!branch.selected) {
                        		scope.checkParent(branch);
                        	}
                        	
                        	scope.treeDataSelect = [];
                        	for_each_branch(function (b) {
                        		if(b.selected) {
                        			scope.treeDataSelect.push(b);
                        		}
                        	});
                        	console.log(scope.treeDataSelect);
                        }
                        
                        scope.checkParent = function(branch) {
                        	var _parent = get_parent(branch);
                        	if(_parent) {
                        		_parent.selected = branch.selected;
                        		scope.checkParent(_parent)
                        	}
                        }
                        
                        scope.checkChildren = function(branch) {
                        	if(branch.children) {
                        		for(var _i = 0; _i<branch.children.length; _i++) {
                        			branch.children[_i].selected = branch.selected;
                        			if(branch.children[_i].children) {
                        				scope.checkChildren(branch.children[_i]);
                        			}
                        		}
                        	}
                        }
